/**
 * Importing the necessary models and dependencies required for recipe handling.
 */
...
const createRecipe = async (req, res) => {
    if (
        ...
        return res.status(400).json({ "message": "Missing required fields to continue" });
    }

    if (isNaN(req.body.recipecallories)){
        await fs.unlink(path.join(__dirname,"..","uploads",req.fileName));
        return res.status(400).json({ "message": "Recipe calories must be a number" });
    }
