...
const getRecipeComments = async (req, res) => {
    ...
    const comments = await Comment.find({ recipeId: req.params.id }).exec();
...
const updateComment = async (req, res) => {
    ...
    if (findComment.author !== req.user) {
        return res.status(401).json({ "message": `You have no permissions to edit the comment` });
    }

    const findRecipe = await Recipe.findOne({ _id: findComment.recipeId }).exec();
    if (req.body?.data?.comment)
        findComment.comment = req.body.data?.comment;
...
const postRecipeComment = async (req, res) => {
    ...
    if (req.user === findRecipe.author)
        return res.status(403).json({ "message": "Author cannot comment on their own recipe" });
